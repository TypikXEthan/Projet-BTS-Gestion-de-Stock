        ...
        # --- MODIFIER UTILISATEUR (AVEC SALT) ---
         ...
         # --- SUPPRIMER UTILISATEUR ---
            ...
            if int(id_u) == session.get('id_user'):
                ...
                try:
                    # Une seule requête directe : la BDD s'occupe du reste en CASCADE
                    cursor.execute("DELETE FROM utilisateurs WHERE id_utilisateur = %s", (id_u,))
                    db.commit()
                    flash("Utilisateur supprimé.", "warning")
                except mysql.connector.Error as err:
                    db.rollback()
                    flash(f"Impossible de supprimer l'utilisateur : {err}", "danger")
        ...
        if user:
            ...
            try:
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                """, (user['id_utilisateur'], 'Porte_Stock', date_str, heure_str, 'Autorisé'))
                db.commit()
                print(f"✅ INSERT historique_acces OK pour {prenom_f} {nom_f}")
            except Exception as insert_err:
                print(f"❌ ERREUR INSERT historique_acces : {insert_err}")
            ...
            try:
                cursor.execute("""
                    INSERT INTO historique_acces (id_utilisateur, id_porte_physique, date_acces, heure_acces, statut_acces)
                    VALUES (%s, %s, %s, %s, %s)
                """, (None, 'Porte_Stock', date_str, heure_str, 'Refusé'))
                db.commit()
                print(f"✅ INSERT historique_acces OK — badge inconnu")
            except Exception as insert_err:
                print(f"❌ ERREUR INSERT historique_acces : {insert_err}")
            ...
            try:
                ...
                if liste_destinataires:
                    msg = Message(
                        ...
                        recipients=liste_destinataires,

...
#Profils utilisateur publique
@app.route("/profil/<int:id_user_vise>")
def voir_profil_public(id_user_vise):
    if "id_user" not in session:
        return redirect("/")

    # Si l'utilisateur clique sur lui-même, on le redirige vers sa page de gestion perso
    if id_user_vise == session["id_user"]:
        return redirect("/profil")

    db = get_db()
    cursor = db.cursor(dictionary=True)

    # 1. Infos de base de l'utilisateur (on ne prend PAS l'identifiant ni le mdp)
    cursor.execute("SELECT nom, prenom, email, telephone, role FROM utilisateurs WHERE id_utilisateur = %s", (id_user_vise,))
    user_vise = cursor.fetchone()

    if not user_vise:
        return "Utilisateur non trouvé", 404

    # 2. Matériel en sa possession
    cursor.execute("SELECT nom_modele, rfid_tag_epc FROM materiel_stock WHERE id_utilisateur_actuel = %s", (id_user_vise,))
    materiels = cursor.fetchall()

    # 3. Réservations
    cursor.execute("""
        SELECT m.nom_modele, r.date_reservation, r.date_rendu_prevue 
        FROM reservations r
        JOIN materiel_stock m ON r.id_materiel = m.id_materiel
        WHERE r.id_utilisateur = %s AND r.statut = 'en_attente'
    """, (id_user_vise,))
    ...
    return render_template("IHM/profil_public.html", 
                           u=user_vise, 
                           materiels=materiels, 
...
# --- MATÉRIEL ---
        # --- MATÉRIEL ---
            ...
            # Note : Dans ton HTML l'input s'appelle "id_inventaire", 
            # on l'utilise pour remplir la colonne "id_materiel"
            id_m = request.form.get("id_inventaire") 
            nom_m = request.form.get("nom_modele")
            rfid_c = request.form.get("rfid_tag")
            ...
            try:
                cursor.execute("""
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (id_m, nom_m, rfid_c, etat_m, 1, 1)) # 1 et 1 pour actif et reservable par défaut
                db.commit()
                flash(f"Matériel '{nom_m}' ajouté (ID: {id_m}).", "success")
            except mysql.connector.Error as err:
                flash(f"Erreur lors de l'ajout : {err}", "danger")
            ...
            cursor.execute("""
                ...
                SET nom_modele=%s,
                    rfid_tag_epc=%s,
                    etat=%s,
                    id_utilisateur_actuel=%s
                WHERE id_materiel=%s
            """, (
                request.form.get("nom_modele"),
                request.form.get("rfid_tag_epc"),
                request.form.get("nouveau_statut"),
                u_act,
                id_m
            ))
            db.commit()
...
#----------------------------
# --- ROUTES TABLETTE ---
@socketio.on('join_tablette')
def handle_join_tablette(data):
    """Permet à la tablette de rejoindre une room dédiée pour optimiser les envois"""
    from flask_socketio import join_room
    join_room('room_tablette')
    print("📱 [SocketIO] La tablette a rejoint la room : room_tablette")
...
def verifier_acces():
    ...
    try:
        ...
        if user:
            ...
            # Envoi des infos à la tablette (seulement à la room tablette pour éviter de ramer)
            ...
            socketio.emit('resultat_badge', {'status': 'rouge'}, to='room_tablette')
            return jsonify({"status": "refuse"}), 403

    except Exception as e:
        print(f"Erreur Serveur Accès: {e}")
...
def valider_session_finale():
    ...
    # 1. Cas où rien n'est sélectionné
    if not tags_selectionnes:
        return jsonify({"status": "ok", "message": "Aucun matériel validé"})
    ...
    try:
        ...
        for tag in tags_selectionnes:
            if tag in session_scan:
                ...
                """, (infos['etat'], nouvel_utilisateur, tag))
                ...
                # Suppression de la mémoire seulement si validé
